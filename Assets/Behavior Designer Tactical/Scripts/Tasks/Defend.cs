    ...
    public class Defend : NavMeshTacticalGroup
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (!tacticalAgent.AttackPosition) {
                ...
                tacticalAgent.transform.GetComponent<Unit>().SetTaskStatus(TASKNAME + "=: Defend Object: " + defendObject.Value.name + ":" + defendObject.Value.transform.position + ", targetPosition " + targetPosition + " HasArrived: " + tacticalAgent.HasArrived() +  " , moving distance " + (int) Vector3.Distance(tacticalAgent.transform.position, targetPosition) );
